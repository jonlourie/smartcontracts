...
export default function NFTPhotoUploader({addedPhotos, onChange}) {
    ...
    //so we use the filter function in react 
    ...
    return(
        <>
            <div className="flex gap-2">
                        ...
                        <button onClick={addedPhotoByLink} className="bg-gray-200 grow px-4 rounded-2xl">Add&nbsp;photo</button>
                        </div>
                        <input type="file" className="hidden"/>
                        <div className="grid mt-2 gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                            {addedPhotos.length > 0 && addedPhotos.map(link => (
                                <div className="h-32 flex relative" key={link} >
                                    <img className="rounded-2xl w-full object-cover position-center" src={"http://localhost:4000/uploads/"+link} alt=""></img>
